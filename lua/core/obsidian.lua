-- Obsidian core module — adapted from Salar's lua/salar/core/obsidian.lua
-- Multi-workspace detection, markdown buffer setup, template datetime patching
local M = {}
...
local function render_obsidian_datetime(format)
  ...
  return rendered:gsub('__OBSIDIAN_LOWER_AMPM__', function()
    return os.date('%p'):lower()
  end)
end

function M.patch_template_substitutions()
  if vim.g.obsidian_template_patch_applied then
    return
  end
  ...
  vim.g.obsidian_template_patch_applied = true
end
...
function M.workspaces()
  -- 1. Check vim.g.obsidian_workspaces (set programmatically if needed)
  ...
  -- 2. Default: your iCloud vault + env-var-based extras
  local candidates = {
    {
      name = 'personal',
      path = env_or('OBSIDIAN_VAULT_PERSONAL', '~/Library/Mobile Documents/iCloud~md~obsidian/Documents/success/'),
    },
    {
      name = 'work',
  ...
  -- 3. Auto-detect any vault the current buffer or cwd sits in
